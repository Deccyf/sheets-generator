...
- [How it works](#how-it-works)
  ...
  - [Sectional Appendix checks](#sectional-appendix-checks)
...
* a **Sectional Appendix** breach, prefixed with that name. This is the one
  kind that is not the tool hedging: every other entry says the reports could
  not settle something, whereas this says a published document forbids the
  working. See [Sectional Appendix checks](#sectional-appendix-checks);
...
| `src/data.js` | **`SHEETS_DATA`** — every reference table for every engine in one module: berths, destination codes, section orders, fleet profiles, the station table, end-marker rules, and the `SA_*` Sectional Appendix checks. Corrections belong here. |
