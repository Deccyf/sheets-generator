...
| `BERTHING SHEET RULES.html` | The rules the tool goes by, written for colleagues who will never open it. Generated by `tools/make-rules-doc.mjs` from the built file's own tables — **nothing on it is typed out separately**, so it cannot drift from what the books do. One self-contained page; open it or print it to PDF. |
...
| `src/rules.js` | **`SHEETS_RULES`** — the pure part of local unit-order corrections (key grammar, merge, export) *and* `explain()`/`explainHtml()`, which turn a build's rules into plain English. The Rules tab and `BERTHING SHEET RULES.html` both render that one call, so the tab, the handout and the books cannot say different things. No DOM, no tables of its own: it takes a plain object describing the build. |
