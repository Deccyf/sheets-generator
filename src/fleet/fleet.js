...
const JOIN = /^(ATTACH|ATTTT)$/i;
const splitsOf = d => d.rows.filter(r => SPLIT.test(r.act || ""))
  .map(r => ({loc: r.loc, t: r.arr ?? r.dep}));

/* ---- who a diagram runs with, and where they come apart ----------------

   The berthing sheets settle this already, and the two tools have to give
   the same answer, so the rules here are theirs:

     - A pair that attaches and detaches but runs the SAME PATH to the same
       berth never really parted. They travel as one train and the events
       are the plan writing down a re-marshalling - flagging that as a split
       would only mislead. Same exception the berthing sheets make.
     - WHO parts from whom. A detach out of a train of three leaves two of
       them still coupled; the unit that leaves is the one that drops out of
       the formation column, not "everybody at that place".
     - WHEN, in the berthing sheets' sense. "PM" there is not a time of day,
       it is a place in the diagram: the units are put away together first
       and the parting comes after that. A pair that parts on the working it
       leaves the depot on is a plain split; one that goes out together,
       works coupled and comes apart later is the depot's SPLITS PM.        */

/* The units named in a formation cell: "601(1)\604(2)" is 601 and 604. */
function formOf(row){
  const out = new Set();
  const re = /(\d+)\s*\(\d+\)/g;
  let m;
  while ((m = re.exec(row.form || "")) !== null) out.add(m[1]);
  return out;
}
/* The formation carried down the diagram, because the column is only
   written where it changes. Each row gets the train it is part of. */
function formationRun(d){
  const me = String(d.num);
  let cur = new Set([me]);
  return d.rows.map(r => {
    const f = formOf(r);
    if (f.size) cur = f;
...
/* Every stop this diagram makes, as "where@when", so two diagrams can be
   asked whether they went the same way. */
const pathOf = (d, from, to) => d.rows.slice(from, to + 1)
  .map(r => r.loc + "@" + (r.arr ?? "") + "/" + (r.dep ?? "")).join(">");

/* One diagram's coupling story: who it joins, who it parts from, where and
   when, and whether the parting is a plain split or the PM kind. `byNum`
   maps a diagram number to its diagram, for the same-path test. */
function partings(d, byNum){
  const run = formationRun(d);
  const me = String(d.num);
  const berths = berthsOf(d);
  /* The first time this diagram is put away AFTER it first runs with
     somebody: the line between a split and a PM one. A stand of berthing
     length is what counts - a two-minute call is not putting a unit away. */
  const out = [];
  let joinedAt = null;
  for (let i = 0; i < d.rows.length; i++){
    const r = d.rows[i], act = r.act || "";
    const with_ = new Set(Array.from(run[i]).filter(n => n !== me));
    /* The MOST RECENT join, not the first: "were they put away together
       since they last coupled up" is the question, so a berth from an
       earlier coupling must not answer it for a later one. A diagram that
       starts coupled has no join event and leaves this null, which means
       coupled from the off and any berth counts. */
    if (JOIN.test(act) && with_.size) joinedAt = r.arr ?? r.dep;
    if (!SPLIT.test(act)) continue;
    const before = run[i];
    const after = i + 1 < run.length ? run[i + 1] : new Set([me]);
    // whoever was in the train and is not any more
    const lost = Array.from(before).filter(n => n !== me && !after.has(n)).sort();
    if (!lost.length) continue;
    const t = r.arr ?? r.dep;
    /* Put away together first? A berth of attendable length before this
       parting, with somebody else still in the train, is the units being
       stabled as one - and then the parting is the PM kind. */
    let stabledTogether = false;
    for (const b of berths){
      if (b.to == null || t == null || b.to > t) break;
      if (b.mins != null && b.mins >= ATTENDABLE &&
          (joinedAt == null || b.from >= joinedAt)) stabledTogether = true;
    }
    out.push({loc: r.loc, t, lost, kind: stabledTogether ? "pm" : "split", i});
  }
  /* …and the exception: a partner that leaves and comes back having gone
     exactly where this diagram went never really parted from it. */
  return out.filter(x => {
    x.lost = x.lost.filter(n => {
      const o = byNum && byNum.get(n);
      if (!o) return true;
      return pathOf(d, x.i, d.rows.length - 1) !== pathOf(o, 0, o.rows.length - 1);
    });
    return x.lost.length > 0;
  });
}
...
/* ---- mileage, per unit and per sub-fleet -------------------------------
   ...
   racks up between them: exams fall due on a unit's clock. So the day's
   miles are shared over the units that could be working them, and those
   daily figures are added across a week and annualised.

   WHICH UNITS, though, is the whole question, and the answer is the FLEET,
   not the diagram book. A diagram book has no spare and no exam float in
   it: the plan needs 27 diagrams and the depot owns 30 units, and all 30
   take their turn through those diagrams. Divide by the diagrams and every
   figure comes out about a ninth too high, which is what the manual sheets
   disagreed with. So the fleet size is a SETTING - the prints cannot know
   it, because a unit standing spare all week never appears in them - and
   the divisor is the size where one is set.

   Both are worth having and both are shown. Per DIAGRAMMED unit answers
   "what does a unit in traffic do"; per FLEET unit answers "what does a
   unit on the books accrue", which is the one exams are planned off.
   Without a fleet size the two are the same figure, and the page says so
   rather than quietly presenting the smaller fleet as the real one.
...
function mileage(all, fleet, monday, sizes){
  ...
  const sizeOf = sub => {
    const v = sizes && sizes[sub];
    return Number.isFinite(+v) && +v > 0 ? +v : null;
  };

  const measure = (label, pick, owned) => {
    ...
    /* The units the PLAN needs, which is the busiest day's diagram count.
       ...
       diagram book, so the real fleet is always larger, and `owned` is
    ...
    return {sub: label, perDay, units, owned, over,
            ...
            weeklyPerOwned,
            annualPerOwned: owned ? weeklyPerOwned * WEEKS : null,
            weeklyTotal, annualTotal: weeklyTotal * WEEKS};
  };

  const rows = subs.map(s => measure(s, d => d.fleet === s, sizeOf(s)));
  /* The fleet's own size is the sub-fleets added up, and only where every
     one of them has been given a size: a part-filled total would read as
     the whole fleet and be wrong by however many are missing. */
  const allSized = subs.length && subs.every(s => sizeOf(s) !== null);
  const ownedAll = allSized ? subs.reduce((t, s) => t + sizeOf(s), 0) : null;
  return {rows, sized: subs.filter(s => sizeOf(s) !== null).length, subs,
...
function analyse(all, fleet, cfg){
  ...
  /* Where diagrams actually come apart - over the WHOLE week, not one
     Monday. An FSX book is Monday to Thursday and the Friday, Saturday and
     Sunday books are different diagrams entirely, so a Monday-only answer
     is silent about three days in seven. Every day is walked and each
     parting is recorded against the days it happens on. */
  ...
  for (const ms of weekFrom(monday)){
    ...
    for (const d of on)
      for (const s of partings(d, byNum)){
        const g = groupOf(s.loc);
        if (!splitAt.has(g))
          splitAt.set(g, {loc: g, n: 0, ds: new Set(), days: new Set(),
                          split: 0, pm: 0});
        const r = splitAt.get(g);
        r.n++; r.ds.add(d.key); r.days.add(nm);
        if (s.kind === "pm") r.pm++; else r.split++;
        partingRows.push({day: nm, d: d.key, num: String(d.num), loc: g,
                          raw: s.loc, t: s.t, lost: s.lost, kind: s.kind});
      }
  }
  /* The AM/PM question the depot asks: which pairs go out together in the
     morning, work the day coupled, and only come apart later. That is the
     berthing sheets' SPLITS PM - the units are put away as one first and
     the parting is still to come - and it is the case a planner has to
     watch, because the unit is not free until the afternoon. */
  const pmSplits = partingRows.filter(x => x.kind === "pm");
  const amSplits = partingRows.filter(x => x.kind === "split");
  ...
  const miles = mileage(all, fleet, monday, c.sizes);
