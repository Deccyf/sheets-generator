...
function parseOperating(txt) {
  ...
  for (const raw of txt.split("\n")) {
    ...
    let i=9;
    if (/^(?:375\/[36789]|377\/\d)$/.test(t[i]||"")) row.allocated=t[i++];
    if (/^(?:375[36789]\d\d|377\d{3})$/.test(t[i]||"")) row.resource=t[i++];
...
return { run, sniff, parseOperating, parseDetail, parseDetailCsv, detailFrom,
         buildDiscrepancies, pdfText, ABBR, MASTER_GROUPS };
})();
