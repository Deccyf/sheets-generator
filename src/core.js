...
function bracketOf(name) {
  const m = /\[([^\]]+)\]/.exec(pyStr(name === null || name === undefined ? "" : name));
...
function binfo(sheetTitle) {
  const k = norm(sheetStation(sheetTitle));
...
function bracketDest(destRaw) {
  const m = /\[([^\]]+)\]/.exec(pyStr(destRaw === null || destRaw === undefined ? "" : destRaw));
  return m ? strip(m[1]) : null;
}
...
function clsText(slot) {
  const c = (slot.cls || "?").replace(/\//g, "-");
  return `${slot.length !== null && slot.length !== undefined ? slot.length : "?"} ${c}`;
}
