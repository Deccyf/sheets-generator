...
function unitDay(unit, genius, date) {
  ...
  return {
    unit, diags, rows, stops, splits, splitsAt,
...
function suggest(r) {
  ...
  // nothing today: where it ends, and any standing fleet move from there
  s.action = "ENDS " + r.ends.place;
...
function run(planText, genius, opts) {
  ...
  return { rows: out, tiered, order: plan.order, reviews, date, today, lines: out.length,
...
return { run, render, shape, toText, toHtml, parsePlan, whenOf, suggest,
