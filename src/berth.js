...
function suggest(r, ctx) {
  ...
  s.action = (r.standing ? "AT " : "ENDS ") + r.ends.place;
...
function run(planText, genius, opts) {
  ...
  for (const r of dated.slice().sort(byPriority)) {
    ...
    r.suggest = suggest(r, { mine: day, days, tomDays, wanted, taken, mse, keep: !!opts.keep, dayToRun: !!opts.dayToRun });
    if (r.suggest.taken) {
  ...
  for (const r of out.slice().sort(byPriority))
    if (!asked.has(r.unit) && /BERTH|C\/O|HOLD/.test(r.suggest.action)) asked.set(r.unit, r.suggest.action);
  for (const r of out)
    if (asked.has(r.unit) && /^(ENDS|AT) /.test(r.suggest.action)) r.suggest.notes.push("asked for on its other line — " + asked.get(r.unit));
