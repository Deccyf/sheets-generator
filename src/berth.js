...
function sheetLine(pool, place, diag, hc, dep) {
  ...
  const fmt = e => HEADCODE_DEPOTS.has(depot) ? (e.headcode || null)
                 : typeof SHEETS_CORE !== "undefined" && SHEETS_CORE.fmtTime ? SHEETS_CORE.fmtTime(e.time, e.time_kind) : null;
  const name = fmt(best);
  // two lines printing the same time: the sheet tells them apart by where they go, and so does the request
  const twins = !!name && all.filter(e => e !== best && fmt(e) === name && (e.dest || "") !== (best.dest || "")).length > 0;
  return name ? { name, dest: best.dest || "", twins } : null;
}
...
function departuresFrom(r, pool, from0, targets, taken, opts) {
  ...
  const RANK = { ends: 0, stands: 1, calls: 2, hub: 3, contain: 4 };
  const byKey = new Map();
  ...
  for (const d of pool.values()) {
    ...
    for (const o of outs) {
      ...
      if (!how && opts.contain) {
        /* multiple only with days to run is contained: on a multiple
           diagram off its road, coupled to a depot stand or to the end of
           the day, wherever that is - back to Ashford will do */
        const st = depotStands(rest.stops).find(x => !NO_REQUEST_AT.has(x.depot));
        if (st) { how = "contain"; via = st.depot; when = st.arr; }
        else if (rest.endTime != null) { how = "contain"; via = rest.endDepot || rest.endPlace; when = rest.endTime; }
      }
      if (!how) continue;
      ...
      if (!byKey.has(wid)) {
        ...
        byKey.set(wid, { wid, hc: o.hc, dep: o.dep, start: !o.idx, road: o.code, sharing, diags: [], reach: [], dest: line ? line.dest : "", twins: !!(line && line.twins),
  ...
  /* multiple only: two diagrams or more of the train get there together -
     the same place at the same time - not one portion at nine and the
     other after midnight */
  const together = (x, y) => x.how === y.how && x.via === y.via && Math.abs((x.when || 0) - (y.when || 0)) <= 5;
  const fitMO = es => es.filter(e => !opts.mo || e.diags.filter(x => together(x, e.diags[0])).length >= 2);
  ...
  const twins = e => (e.twins || listed.filter(x => x.name === e.name).length > 1) && e.dest;
  ...
  const whereTo = x => x.how === "contain" ? "contained — in multiple to " + x.via + " " + hhmm(x.when, true)
                     : (x.how === "ends" ? (atHome ? "back " : "ends ") : x.how === "stands" ? (atHome ? "back " : "stands ") : x.how === "calls" ? "calls " : x.ends ? "ends " : x.via === from ? "back " : "stands ") +
...
function suggestCore(r, ctx) {
  ...
  const listFrom = (from, extra) => {
    ...
    return departuresFrom(r, pool, from, targets, ctx.taken, { ...base, proxy, code: r.standing ? (r.standingRoad || null) : r.ends.code, mates: matesOn(ctx.mine, null, ctx.days), ...(extra || {}) });
  };
  const useList = lst => {
    s.action = lst.action;
...
/* Where the plan's own request puts a unit: "GP BERTH FP 5S07" is a unit
   on the road 5S07 leaves from - the Up Sidings - so the road is read
   off the working named, where the day's Detail has it. */
function roadFromAction(action, place, pool) {
  const m = /^[A-Z]+ BERTH ([0-9A-Z+ \/]+?)(?: THEN.*)?$/.exec(String(action || "").trim().toUpperCase());
  if (!m || !pool || !pool.size) return null;
  // every working named: a list off two roads places the unit on neither
  const names = m[1].split("/").map(x => x.replace(/^[FMR]P /, "").replace(/ (VIC|RAM|CST|CHX|GI|SG|GP)$/, "").trim()).filter(Boolean);
  const codes = codesAt(place), roads = new Set();
  for (const name of names) {
    let road = null;
    for (const d of pool.values()) {
      const s0 = d.stops[0];
      if (!s0 || codes.indexOf(s0.code) < 0 || !s0.hcOut || s0.dep == null) continue;
      if (s0.hcOut === name || sheetName(pool, place, d.diag, s0.hcOut, s0.dep) === name) { road = s0.code; break; }
    }
