...
/* ==== Genius location knowledge (ex GENIUS) ==== */
  ...
  const ORDER_FIX = {
    ...
    /* RM013 LEADS from Ashford to Dover Priory, per the depot, off the
       18/09 book's 03+52. Ashford reads highest Position first, 013 is
       Position 1, so unpinned it trails.

       This is a pin rather than a rule because nothing in either report
       separates it from the 05 22 to the same place. Both come out of the
       Down Sidings, both run the yard to the platform and away to Dover
       Priory, both split later, and the Summary gives both their Position
       off the same berth. The one field that does differ is the one the
       depot named: 5R05 runs THROUGH the platform as one working, where
       5R02 becomes 2R02 there and goes into service. One example is not a
       rule, and reading it as one would turn the 05 22 round too - which
       would put 301 in front of 901, against what this table already
       records for that formation. If the depot says the 05 22 should move
       as well, that is the second example, and then it is a rule. */
    "ASHFORD|013,014": ["013", "014"],
    "ASHFORD|114,115": ["115", "114"],
    /* Both units come through the reports on Position 1 for this working, so
       the numbers cannot separate them at all and the order was falling to
       the diagram tie-break, which carries no meaning of its own. It is the
       only coupled entry whose Positions are not 1..n on either properly
       sectioned day.

       128 LEADS and 117 TRAILS, per the depot.

       Note for whoever reads this next: the hand-written TUE 18/08 book has
       it the other way up, 117 on the first row of its 15 27 and 128 on the
       second (rows 39 and 40). The depot says that book is wrong here, and
       the depot is the authority on which end of a train is which - a
       hand-kept book carrying an error is one of the reasons this tool
       exists. So the entry-by-entry check against that book now reports this
       one as a difference on purpose; it is not a regression. */
    "ASHFORD|117,128": ["128", "117"],
    "ASHFORD|301,901,902": ["901", "902", "301"],
    "ASHFORD 15+43|101,102": ["101", "102"],


    /* 021,022 used to be pinned here, ["022","021"], off the hand-written
       12/08 book. The depot says that row is wrong, the same way it said the
       Ashford 117/128 one is: RM021 is Position 1 off the Grove Park Up
       C.H.S and leads. The check is the 18/09 book, where SG811/SG812 leave
       the SAME road on the next working out of it (5F92 at 17+00, against
       021/022's 5F34 at 15+18) and print Position 1 first, unpinned, because
       GROVE PARK is on the posAsc list. One road cannot face two ways, so
       with the pin gone the section rule gives both pairs the same reading
       and both agree with the depot. */
  ...
  /* SVNOCHS, the Sevenoaks siding, is the fifth and reads exactly like the
     others: nothing is based there, and the only call on it in the 18/09
     reports is RM043 running in off the 5Z37 at 10:42 and straight back out
     on the 5Z39 at 10:56 - fourteen minutes, a turnback on a route-learning
     move. Counted a berthing it opened a SEVENOAKS page in the book for that
     one line, and it took the column D of RM043's Grove Park 09+58 entry
     with it: the unit's next real berth is back at the Grove Park down
     sidings at 16+19, which is GPD, and the depot asked for GPD. */
  ...
  /* The same spurs under the names the weekend PRINTS use for them, so the
     ...
     The Sevenoaks siding has no line here on purpose: no weekend print seen
     so far calls at it, so nothing says what those documents spell it, and
     a guessed spelling would sit in a lookup table reading like a fact. Add
     it the day a print shows it. */
...
return {
  ...
  TRANSIT, STATIONS, MANUAL_LOC, END_MARKERS_PRINTS,
