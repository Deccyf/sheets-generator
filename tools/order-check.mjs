...
const rows = [["Section", "Time", "To", "Headcode", "Sheet shows",
               "The other way round", "Which is right? (A or B)", "Notes"]];
const ORDER = N.SHEETS_XLSX.MAIN_ORDER;
const secs = res.secsByDay.M;
...
for (const sec of seen) {
  for (const e of secs.get(sec)) {
    ...
    rows.push([sec, t, e.dest, e.headcode || "",
      "A: " + shown.join(" then "),
...
console.log("wrote " + n + " coupled entries");
